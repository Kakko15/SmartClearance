...
import { ClockIcon, CheckCircleIcon, XCircleIcon, DocumentCheckIcon, InboxStackIcon } from "../ui/Icons";
...
export default function StudentRequestHistory({ studentId, isDarkMode = false }) {
        ...
        {[1, 2, 3].map((i) => (
          <GlassCard key={i} isDark={isDarkMode} className="p-5 rounded-2xl">
            <div className="flex items-center gap-4">
              <div className={`w-10 h-10 rounded-full animate-pulse ${isDarkMode ? "bg-[#3c4043]" : "bg-[#e8eaed]"}`} />
              <div className="flex-1">
                <div className={`h-4 w-40 rounded animate-pulse ${isDarkMode ? "bg-[#3c4043]" : "bg-[#e8eaed]"}`} />
                <div className={`h-3 w-24 rounded mt-2 animate-pulse ${isDarkMode ? "bg-[#3c4043]" : "bg-[#e8eaed]"}`} />
              </div>
            </div>
          </GlassCard>
        ))}
      ...
      {filtered.length === 0 ? (
        <GlassCard isDark={isDarkMode} className="p-12 text-center rounded-2xl">
          ...
          <p className={`text-sm ${isDarkMode ? "text-[#9aa0a6]" : "text-[#5f6368]"}`}>
            {filter === "all" ? "You haven't submitted any clearance requests yet." : `No ${filter.replace("_", " ")} requests.`}
          </p>
        </GlassCard>
      ) : (
        <div className="space-y-3">
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className={`text-sm font-medium truncate ${isDarkMode ? "text-[#e8eaed]" : "text-[#202124]"}`}>
                        {r.document_types?.name || "Clearance Request"}
                      </p>
                      <div className={`flex items-center gap-2 text-xs mt-0.5 ${isDarkMode ? "text-[#9aa0a6]" : "text-[#5f6368]"}`}>
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <StatusBadge status={r.is_completed ? "completed" : r.current_status} isDark={isDarkMode} />
