...
export default function SelfieCapture({ idDescriptor, onMatch, isDark }) {
    ...
    const handleVisibilityChange = () => {
      ...
      if (
        ...
        if (!isCameraActiveAndValid()) {
          console.log(
            "Restarting camera due to invalid stream state on visibility",
          );
  ...
  useEffect(() => {
    idDescriptorRef.current = idDescriptor;
  }, [idDescriptor]);
  useEffect(() => {
  ...
  const evaluateLiveFaceQuality = useCallback((faceResult) => {
    ...
    if (!box || !landmarks || !frameWidth || !frameHeight) {
      return {
        valid: false,
        message: "Position your full face in the circle.",
      };
    }

    const detectionScore =
      typeof rawDetection?.score === "number" ? rawDetection.score : 1;
    if (detectionScore < LIVE_FACE_RULES.minDetectionScore) {
      return {
        valid: false,
        message: "Hold still and face the camera directly.",
      };
    }
    ...
    if (faceAreaRatio < LIVE_FACE_RULES.minAreaRatio) {
      return {
        valid: false,
        message: "Move closer so your full face fills the guide.",
      };
    }
    ...
    if (
      ...
      return {
        valid: false,
        message: "Align your face to the center of the circle.",
      };
    }
    ...
    if (leftEye && rightEye && nose) {
      ...
      const noseOffsetRatio =
        Math.abs(nose.x - eyeMidX) / Math.max(eyeDistance, 1);
      ...
      if (eyeDistanceRatio < LIVE_FACE_RULES.minEyeDistanceRatio) {
        return {
          valid: false,
          message: "Face the camera directly (avoid side angles).",
        };
      }
  ...
  const startAutoDetection = useCallback(() => {
    ...
    autoDetectRef.current = setInterval(async () => {
      if (!videoRef.current || hasSubmittedRef.current || scanningRef.current)
        return;
  ...
  const performCapture = useCallback(async () => {
    ...
    if (!isWindowActive()) {
      setResult({
        success: false,
        message:
          "Return to the SmartClearance window before capturing your selfie.",
      });
      return;
    }
    ...
    try {
      if (!isWindowActive()) {
        setResult({
          success: false,
          message:
            "Verification paused while another window is active. Please try again.",
        });
        hasSubmittedRef.current = false;
        startAutoDetection();
        setCapturing(false);
      ...
      const { comparisons: sampleComparisons, guidanceMessage } =
        await collectComparisonSamples();
      ...
      const passCount = sampleComparisons.filter(
        (sample) => sample.isMatch,
      ).length;
      const minPassesNeeded = Math.ceil(sampleComparisons.length / 2);
      const isMatch =
        medianSimilarity >= threshold && passCount >= minPassesNeeded;
      ...
      {!consentGiven ? (
        ...
        >
          <div className="text-center mb-6">
            ...
            <p
              className={`text-sm leading-relaxed max-w-md mx-auto ${isDark ? "text-gray-400" : "text-gray-600"}`}
            >
              We need to access your camera to take a live selfie and verify it
              matches the photo on your uploaded ID. This ensures your identity
              is protected.
            </p>
          </div>
          <div
            className={`p-4 rounded-xl mb-6 ${isDark ? "bg-slate-900/50" : "bg-gray-50"}`}
          >
            <p
            ...
            <ul
              className={`text-sm space-y-1.5 ${isDark ? "text-gray-400" : "text-gray-600"}`}
            >
        ...
        >
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  {result.similarity !== undefined && (
                    <div className="mt-3">
                      <div
                        className={`flex justify-between text-[13px] mb-1.5 ${isDark ? "text-[#9aa0a6]" : "text-[#5f6368]"}`}
                      >
          ...
          {failCount >= 3 && result && !result.isMatch && (
            ...
            >
              <div className="flex items-center justify-center gap-2 mb-1">
                <svg
                  className={`w-4 h-4 ${isDark ? "text-[#fbbc04]" : "text-[#ea8600]"}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
          ...
          >
            ...
            >
              ...
              <li
                className={`flex items-start gap-2 font-medium ${isDark ? "text-[#fbbc04]" : "text-[#ea8600]"}`}
              >
                <span className="mt-1.5 flex-shrink-0 w-1 h-1 rounded-full bg-current opacity-80"></span>
                <span>
                  Maximum of 3 attempts — after that, submit to the Registrar
                  for manual review.
                </span>
              </li>
            </ul>
