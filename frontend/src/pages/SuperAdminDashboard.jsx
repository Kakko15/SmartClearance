...
const MetricPill = ({ label, value, icon, color = "indigo", isDarkMode }) => {
  ...
  return (
    <GlassCard
      className="flex flex-col justify-center p-6 min-h-[140px]"
      isDark={isDarkMode}
    >
      <div className="flex items-start mb-4">
        <div
          className={`p-3 rounded-2xl ${bgColors[color]} ${colors[color]} transition-colors duration-300`}
        >
          {icon}
        </div>
      </div>
      <div
        className={`text-3xl font-bold mb-1 tracking-tight ${isDarkMode ? "text-white" : "text-gray-900"}`}
      >
        {value}
      </div>
      <div
        className={`text-xs font-medium uppercase tracking-widest ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}
      >
        {label}
      </div>
    </GlassCard>
...
export default function SuperAdminDashboard({
  ...
  const [activeView, setActiveView] = useState(
    () => sessionStorage.getItem("tab_superadmin") || "dashboard",
  );
    ...
    try {
      const [
        ...
        supabase
          ...
          .select(
            "*, profiles!request_history_processed_by_fkey(full_name, role)",
          )
  ...
  const handleReviewEdit = async (editId, action, comment = "") => {
    ...
    try {
      const { data } = await authAxios.post(`profile/review-edit/${editId}`, {
        action,
        comment,
      });
      if (data.success) {
    ...
    >
      <div className="max-w-5xl mx-auto space-y-8 pb-10">
        {activeView === "dashboard" && (
          <div className="space-y-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h2
                  className={`text-3xl font-bold ${isDarkMode ? "text-white" : "text-gray-900"} mb-1`}
                >
                  Admin Dashboard
                </h2>
                <p
              ...
              {statsLoading ? (
                <div className="space-y-3">
                  {[1, 2, 3].map((i) => (
                    <div
                      key={i}
                      className={`flex items-center gap-4 p-3 rounded-xl border ${isDarkMode ? "bg-slate-800/50 border-slate-700/50" : "bg-slate-50 border-slate-100"}`}
                    >
                      <div
                        className={`w-8 h-8 rounded-full animate-pulse ${isDarkMode ? "bg-slate-700" : "bg-slate-200"}`}
                      />
                      <div className="flex-1 space-y-2">
                        <div
                          className={`w-40 h-3 rounded animate-pulse ${isDarkMode ? "bg-slate-700" : "bg-slate-200"}`}
                        />
                        <div
                          className={`w-24 h-2 rounded animate-pulse ${isDarkMode ? "bg-slate-700" : "bg-slate-200"}`}
                        />
                      </div>
                    </div>
                  ))}
                ...
                <p
                  className={`text-sm text-center py-8 ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}
                >
                  No recent activity
                </p>
              ) : (
                <div className="space-y-3">
                  {recentActivity.map((entry, index) => (
                    <div
                      key={entry.id}
                      className={`flex items-center gap-4 p-3 rounded-xl border ${isDarkMode ? "bg-[#282a2d] border-[#3c4043]" : "bg-white border-[#dadce0] shadow-sm"}`}
                    >
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${getActionBgColor(entry.action_taken, isDarkMode)}`}
                      >
                        <div
                          className={`w-2 h-2 rounded-full ${getActionColor(entry.action_taken).replace("text-", "bg-")}`}
                        />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p
                          className={`text-[14px] truncate ${isDarkMode ? "text-white" : "text-gray-900"}`}
                        >
                          <span
                            className={`font-semibold ${getActionColor(entry.action_taken)} mr-1`}
                          >
                        ...
                        {entry.comments && (
                          <p
                            className={`text-[13px] truncate mt-0.5 ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}
                          >
                            "{entry.comments}"
                          </p>
                        )}
                      ...
                      <span
                        className={`text-[12px] flex-shrink-0 font-medium ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}
                      >
        ...
        {activeView === "all-users" && (
          <AllUsersView adminId={adminId} isDark={isDarkMode} />
        )}

        {activeView === "secret-codes" && (
          <SecretCodesManager isDark={isDarkMode} />
        )}
            ...
            {analyticsError ? (
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                  <MetricPill
                    label="Pending"
                    value={analytics.requests.pending}
                    color="amber"
                    icon={<DocumentTextIcon className="w-6 h-6" />}
                    isDarkMode={isDarkMode}
                  />
                  <MetricPill
                    label="Completed"
                    value={analytics.requests.completed}
                    color="emerald"
                  ...
                  <MetricPill
                    ...
                    value={
                      analytics.avgCompletionHours < 24
                        ? `${analytics.avgCompletionHours}h`
                        : `${Math.round(analytics.avgCompletionHours / 24)}d`
                    }
                    color="indigo"
                ...
                {analytics.bottlenecks.length > 0 && (
                  <GlassCard className="p-6" isDark={isDarkMode}>
                    ...
                    <p
                      className={`text-sm mb-5 ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}
                    >
                      Stages with the most pending requests
                    </p>
                    <div className="space-y-4">
                      {analytics.bottlenecks.map((b) => (
                        <div key={b.stage} className="flex items-center gap-4">
                          <span
                            className={`text-[14px] font-medium w-28 capitalize ${isDarkMode ? "text-slate-300" : "text-gray-700"}`}
                          >
                            {b.stage}
                          </span>
                          <div
                            className={`flex-1 h-2.5 rounded-full overflow-hidden ${isDarkMode ? "bg-slate-700" : "bg-slate-100"}`}
                          >
                            <motion.div
                              initial={{ width: 0 }}
                              animate={{
                                width: `${Math.min(100, (b.count / Math.max(...analytics.bottlenecks.map((x) => x.count))) * 100)}%`,
                              }}
                          ...
                          <span
                            className={`text-[14px] font-bold w-10 text-right ${isDarkMode ? "text-white" : "text-gray-900"}`}
                          >
                            {b.count}
                          </span>
                        </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <GlassCard className="p-6" isDark={isDarkMode}>
                    ...
                    <p
                      className={`text-[11px] uppercase tracking-widest font-semibold ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}
                    >
                      Registered Students
                    </p>
                  </GlassCard>
        ...
        {activeView === "profile-edits" && (
          <div className="space-y-6">
            <div>
              ...
              <p className={isDarkMode ? "text-slate-400" : "text-slate-500"}>
                Review and approve student profile changes
              </p>
            </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                      <div className="min-w-0">
                        ...
                        >
                          ...
                          <span
                            className={`${isDarkMode ? "text-slate-500" : "text-slate-400"} font-normal`}
                          >
                            (
                            {edit.profiles?.student_number ||
                              edit.profiles?.role}
                            )
                          </span>
                        </p>
                        <p
                          className={`text-[13px] ${isDarkMode ? "text-slate-300" : "text-gray-700"}`}
                        >
                          <span
                            className={
                        ...
                        <p
                          className={`text-[11px] mt-3 ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}
                        >
