...
export default function ProfessorDashboard({
  ...
  const [activeView, setActiveView] = useState(
    () => sessionStorage.getItem("tab_signatory") || "pending",
  );
  ...
  const handleApprove = async (approvalId) => {
    ...
    try {
      const response = await authAxios.post(`graduation/professor/approve`, {
  ...
  const handleReject = async (approvalId) => {
    ...
    setActionLoading(true);
    try {
      const response = await authAxios.post(`graduation/professor/reject`, {
        approval_id: approvalId,
        professor_id: professorId,
  ...
  const getStudentNumber = (s) => s.request?.student?.student_number || "";
  const getStudentCourse = (s) => s.request?.student?.course_year || "N/A";
  const getStudentEmail = (s) => s.request?.student?.email || "N/A";
  ...
  const filteredDisplayStudents = displayStudents.filter((s) => {
    ...
    if (dateTo && s.created_at && s.created_at > dateTo + "T23:59:59")
      return false;
    return true;
  });
    ...
    >
          ...
          {loading ? (
            <div className="space-y-4">
              {[1, 2, 3, 4].map((i) => (
                <GlassCard
                  key={i}
                  className="overflow-hidden border-none shadow-[0_1px_2px_0_rgba(60,64,67,0.3),0_1px_3px_1px_rgba(60,64,67,0.15)] rounded-[20px] mb-2"
                  isDark={isDarkMode}
                >
                  <div
                    className={`p-4 sm:p-5 transition-colors duration-200 ${isDarkMode ? "bg-[#3c4043]/10" : "bg-[#f8f9fa]"}`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-12 h-12 rounded-full animate-pulse ${isDarkMode ? "bg-[#3c4043]" : "bg-[#e8f0fe]"}`}
                        />
                        <div>
                          <div
                            className={`h-4 w-40 rounded mb-2 animate-pulse ${isDarkMode ? "bg-[#3c4043]" : "bg-[#e8f0fe]"}`}
                          />
                          <div
                            className={`h-3 w-24 rounded animate-pulse ${isDarkMode ? "bg-[#3c4043]" : "bg-[#e8f0fe]"}`}
                          />
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <div
                          className={`hidden sm:block h-6 w-24 rounded-full animate-pulse ${isDarkMode ? "bg-[#3c4043]" : "bg-[#e8f0fe]"}`}
                        />
                        <div
                          className={`w-8 h-8 rounded-full animate-pulse ${isDarkMode ? "bg-[#3c4043]" : "bg-[#e8f0fe]"}`}
                        />
                      </div>
                    </div>
                  </div>
            ...
            <GlassCard className="p-12 text-center" isDark={isDarkMode}>
              ...
              >
                <InboxStackIcon
                  className={`w-10 h-10 ${isDarkMode ? "text-[#8ab4f8]" : "text-[#1a73e8]"}`}
                />
              </motion.div>
              <h3
                className={`text-[20px] font-medium mb-2 ${isDarkMode ? "text-[#e8eaed]" : "text-[#202124]"}`}
                style={{ fontFamily: "Google Sans, sans-serif" }}
              >
                No {activeView} requests
              </h3>
              <p
                className={`text-[15px] ${isDarkMode ? "text-[#9aa0a6]" : "text-[#5f6368]"}`}
              >
                {activeView === "pending"
            ...
            <div className="space-y-4">
              {filteredDisplayStudents.map((student, idx) => (
                ...
                >
                  <GlassCard
                    className="overflow-hidden border-none shadow-[0_1px_2px_0_rgba(60,64,67,0.3),0_1px_3px_1px_rgba(60,64,67,0.15)] hover:shadow-[0_1px_3px_0_rgba(60,64,67,0.3),0_4px_8px_3px_rgba(60,64,67,0.15)] transition-shadow duration-300 rounded-[20px] mb-2"
                    isDark={isDarkMode}
                  >
                    <div
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <h3
                                className={`font-medium text-[16px] leading-tight ${
                                  isDeletedStudent(student)
                                    ? isDarkMode
                                      ? "text-[#9aa0a6]"
                                      : "text-[#9aa0a6]"
                    ...
                    <AnimatePresence>
                      {expandedStudent === student.id && (
                        ...
                        >
                          <div
                            className={`px-4 sm:px-5 pb-5 border-t ${isDarkMode ? "border-[#3c4043]" : "border-[#dadce0]"}`}
                          >
                            {isDeletedStudent(student) ? (
                              <div
                                className={`mt-4 flex items-center gap-3 p-4 rounded-[16px] ${
                                  isDarkMode
                                    ? "bg-[#3c4043]/30 text-[#9aa0a6]"
                              ...
                              >
                                ...
                                <p className="text-[14px]">
                                  This student's account has been removed from
                                  the system. The approval record is preserved
                                  for audit purposes.
                                </p>
                              </div>
                            ) : (
                              <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                                ...
                                >
                                  ...
                                  <p
                                    className={`text-[15px] font-medium mt-1 ${isDarkMode ? "text-[#e8eaed]" : "text-[#202124]"}`}
                                  >
                                ...
                                >
                                  ...
                                  <p
                                    className={`text-[15px] font-medium mt-1 truncate ${isDarkMode ? "text-[#e8eaed]" : "text-[#202124]"}`}
                                  >
                            ...
                            {student.status === "pending" && (
                              <div
                                className={`mt-5 pt-5 border-t ${isDarkMode ? "border-[#3c4043]" : "border-[#dadce0]"}`}
                              >
                                <p
                                  className={`text-[11px] mb-3 font-semibold uppercase tracking-wider ${isDarkMode ? "text-[#9aa0a6]" : "text-[#5f6368]"}`}
                                >
                                  Decision
                                </p>
                                {student.is_locked ? (
                                  <div
                                    className={`flex items-center gap-3 px-4 py-3.5 rounded-[12px] border text-[14px] font-medium ${
                                      isDarkMode
                                        ? "bg-[#3c4043]/50 text-[#9aa0a6] border-[#3c4043]"
                                        : "bg-[#f8f9fa] text-[#5f6368] border-[#dadce0]"
                                    }`}
                                  >
                                    <ClockIcon className="w-5 h-5 flex-shrink-0" />
                                    <span>
                                      Waiting for previous clearance
                                      approvals...
                                    </span>
                                  </div>
                                ) : (
                                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                                    <motion.button
                                      ...
                                      className={`flex items-center justify-center gap-2 px-6 py-2.5 rounded-full font-medium transition-colors disabled:opacity-50 text-[14px] ${
                                        isDarkMode
                                          ? "bg-[#81c995] text-[#0d3b16] hover:bg-[#81c995]/90"
                                          : "bg-[#137333] text-white hover:bg-[#137333]/90"
                                      }`}
                                    >
                                      ...
                                      {selectedRejectId === student.id ? (
                                          ...
                                          <div className="flex gap-2 flex-shrink-0 w-full sm:w-auto">
                                            <motion.button
                                              ...
                                              className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 rounded-full font-medium transition-colors disabled:opacity-50 text-[14px] ${
                                                isDarkMode
                                                  ? "bg-[#f28b82] text-[#5c1010] hover:bg-[#f28b82]/90"
                                                  : "bg-[#c5221f] text-white hover:bg-[#c5221f]/90"
                                              }`}
                                            >
                                              Confirm
                                            </motion.button>
                                        ...
                                        <motion.button
                                          ...
                                          className={`flex items-center justify-center gap-2 px-6 py-2.5 rounded-full border transition-colors text-[14px] font-medium ${
                                            isDarkMode
                                              ? "border-[#f28b82] text-[#f28b82] hover:bg-[#f28b82]/10"
                                              : "border-[#c5221f] text-[#c5221f] hover:bg-[#c5221f]/5"
                                          }`}
                                        >
                            ...
                            {student.status !== "pending" &&
                              student.comments && (
                                <div
                                  className={`mt-5 p-4 rounded-[16px] text-[14px] ${
                                    isDarkMode
                                      ? "bg-[#3c4043]/30 text-[#e8eaed]"
                                      : "bg-[#f8f9fa] text-[#202124]"
                                  }`}
                                >
                                  <div className="flex items-center gap-2 mb-2">
                                    <ChatBubbleIcon
                                      className={`w-4 h-4 ${isDarkMode ? "text-[#9aa0a6]" : "text-[#5f6368]"}`}
                                    />
                                    <span
      ...
      {activeView === "delegation" && (
        <div className="max-w-3xl mx-auto space-y-6">
          <div>
            <h2
              className={`text-[28px] leading-tight font-medium tracking-tight ${isDarkMode ? "text-[#e8eaed]" : "text-[#202124]"}`}
              style={{ fontFamily: "Google Sans, sans-serif" }}
            >
              Delegation Management
            </h2>
            <p
              className={`text-[15px] mt-1 ${isDarkMode ? "text-[#9aa0a6]" : "text-[#5f6368]"}`}
            >
