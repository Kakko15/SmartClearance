...
import { getCashierTheme } from "../constants/dashboardThemes";
...
export default function CashierAdminDashboard({
  ...
  const fetchPendingRequests = useCallback(async (silent = false) => {
    ...
    try {
      const response = await authAxios.get(`graduation/cashier/pending`);
                ...
                <GlassCard className="p-5" isDark={isDarkMode}>
                  ...
                  <div className="mb-4">
                    ...
                    <textarea
                      ...
                      rows={3}
                      className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all resize-none ${isDarkMode ? "bg-slate-800 border-slate-600 text-white placeholder-gray-500" : "bg-white/60 border-gray-200 text-gray-900"}`}
                    />
                  </div>
