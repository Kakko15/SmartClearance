...
export const updateClearanceComment = async (
  commentId,
  userId,
  commentText,
) => {
...
export const getEscalationStats = async (adminId) => {
  const headers = await getAuthHeaders();
  return apiFetch(`${API_URL}/escalation/stats?admin_id=${adminId}`, {
    headers,
  });
};
...
export const createSecretCode = async (
  role,
  description,
  maxUses,
  expiresAt,
...
export const toggleSecretCode = async (id) => {
  const headers = await getAuthHeaders();
  return apiFetch(`${API_URL}/admin/secret-codes/${id}/toggle`, {
    method: "PATCH",
    headers,
  });
};
...
export const deleteSecretCode = async (id) => {
  const headers = await getAuthHeaders();
  return apiFetch(`${API_URL}/admin/secret-codes/${id}`, {
    method: "DELETE",
    headers,
  });
};
