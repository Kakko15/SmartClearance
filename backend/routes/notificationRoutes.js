...
// GET /api/notifications/pending-count — count pending requests for staff role
router.get("/pending-count", requireAuth, async (req, res) => {
  try {
    ...
    if (role === "librarian") {
      // Library: only count requests where prerequisite stages are done
      const { data: requests } = await supabase
        .from("requests")
        .select("id, cashier_status, professor_approvals(status, professor:professor_id(full_name))")
      ...
      const { data: requests } = await supabase
        .from("requests")
        .select("id, library_status, professor_approvals(status, professor:professor_id(full_name))")
        .eq("clearance_type", "graduation")
      ...
      count = (requests || []).filter((r) => {
        ...
        return true; // Graduate: cashier is first step
      }).length;
      ...
      // Only count approvals that are actually unlocked (prerequisites met)
      ...
      count = (approvals || []).filter((app) => {
        ...
        // Interleaved admin stage checks
        if (myName === "NSTP Director") {
          if (app.request.library_status !== "approved" || app.request.cashier_status !== "approved") return false;
        } else if (myName === "Executive Officer") {
          if (app.request.cashier_status !== "approved") return false;
        } else if (myName === "Dean Graduate School") {
          if (app.request.cashier_status !== "approved" || app.request.library_status !== "approved" || app.request.registrar_status !== "approved") return false;
        }
