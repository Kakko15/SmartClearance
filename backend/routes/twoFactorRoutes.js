...
const SIGNUP_TOKEN_EXPIRY_MS = 30 * 60 * 1000;
...
async function validateSignupToken(userId, token) {
  ...
  if (!stored)
    return {
      valid: false,
      reason: "No signup token found. Please sign up again.",
    };
  if (Date.now() > stored.expiresAt) {
    await deleteToken(userId, TOKEN_TYPES.SIGNUP_TOKEN);
    return {
      valid: false,
      reason: "Signup token expired. Please sign up again.",
    };
  }
  if (stored.tokenValue !== token)
    return { valid: false, reason: "Invalid signup token." };
...
try {
  ...
  t.verify()
    ...
    .catch((err) => {
      console.warn(
        "[SMTP] Warm-up failed (will retry on first send):",
        err.message,
      );
    });
} catch (smtpInitErr) {
...
router.post("/setup", optionalAuth, async (req, res) => {
  try {
    ...
    if (!isAuthorized) {
      return res
        .status(401)
        .json({ success: false, error: "Unauthorized to setup 2FA" });
    }
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ success: false, error: "Failed to save 2FA secret" });
    }
...
router.post("/verify-setup", optionalAuth, async (req, res) => {
  try {
    ...
    if (!userId || !token) {
      return res
        .status(400)
        .json({ success: false, error: "User ID and token are required" });
    }
    ...
    if (!isAuthorized) {
      return res
        .status(401)
        .json({ success: false, error: "Unauthorized to verify 2FA setup" });
    }
    ...
    if (profileError || !profile?.totp_secret) {
      return res
        ...
        .json({
          success: false,
          error: "2FA not initialized. Please restart setup.",
        });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (!loginResult || !loginResult.valid) {
        ...
        return res
          .status(400)
          .json({ success: false, error: "Invalid code. Please try again." });
      }
    ...
    try {
      ...
      if (existing && Date.now() - existing.createdAt < cooldownMs) {
        const waitSec = Math.ceil(
          (cooldownMs - (Date.now() - existing.createdAt)) / 1000,
        );
      ...
      const nextCooldownMs = Math.min(
        30000 * Math.pow(2, newResendCount),
        300000,
      );
  ...
  async (req, res) => {
    try {
      ...
      if (!userId || !otp) {
        return res
          .status(400)
          .json({ success: false, error: "User ID and OTP are required" });
      }
      ...
      if (!stored) {
        console.error(
          "Email OTP not found for user:",
          userId,
          "- token may have expired or was never created",
        );
        return res
          .status(400)
          .json({
      ...
      if (stored.tokenValue !== otp.toString()) {
        console.error(
          "Email OTP mismatch for user:",
          userId,
          "expected length:",
          stored.tokenValue.length,
          "got length:",
          otp.toString().length,
        );
        const newAttempts = await incrementAttempts(
          userId,
          TOKEN_TYPES.EMAIL_OTP,
        );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to reset authenticator" });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (!pending || !pending.tokenValue) {
        return res
          ...
          .json({
            success: false,
            error:
              "No pending reset found. Please start the reset process again.",
          });
      }

      const result = verifySync({
        token: token.toString(),
        secret: pending.tokenValue,
        epochTolerance: 30,
      });
      ...
      if (error) {
        return res
          .status(500)
          .json({ success: false, error: "Failed to save new authenticator" });
      }
